...
class ChooseView extends PureComponent {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    let { listImg, modelSelect } = nextProps;
    if (!prevState) {
      return {
        offsetX: 0,
        allWidth: 30000,
  ...
  componentDidMount() {
    // 可是窗口宽度
    let viewingAreaWidth = (this.viewingAreaWidth = this.viewingArea.clientWidth);
    // 每一个子项宽度
    let itemWidth = this.itemContainer.children[0].offsetWidth;
    // 步长
    this.itemStepWidth = itemWidth + 16;
    // 所有子项的宽度之和
    let allItemWidth = itemWidth * this.props.listImg.length;
    // 所有间距之和
    let allItemMargin = 16 * (this.props.listImg.length - 1);
    // 滚动区域宽度
   ...
   */
  previous = () => {
    let { offsetX, preDisabled } = this.state;
   ...
   */
  next = () => {
    let { offsetX, nextDisabled } = this.state;
